...
private:
	short aliados_cant, asesinos_cant, corruptos_cant, portales_cant;
...
public:
	...
	void reiniciar_lab() {
		...
		for (short i = this->config->get_asesinos_cant() - 1; i > 0; i--) {
			asesinos->RemoveAt(i);
		}
		for (short i = this->config->get_portales_cant() - 1; i > 0; i--) {
			portales->RemoveAt(i);
		}
